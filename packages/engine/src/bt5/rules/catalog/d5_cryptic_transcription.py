...
The calibration anchor is not hypothetical: the dengue-2 cryptic promoter (-35
`TCAACG` at nt 53, 17 bp spacer, -10 `TTTTTAAT` at nt 72) produced ~10^6.6 mRNA
copies/ug uninduced and made the clone UNCLONEABLE.
...
Five parts, and they do not all have the same shape:
  ...
  (c) the AT-tract rule: `TATTTAT` or `AATTT` whose 3' end sits EXACTLY 5 bp
      upstream of a -10-like hexamer -- 103/103 randomized appropriately
      positioned AT-tracts were active
...
#: `brief.md:111` (c). The tract's 3' end sits exactly this many bases upstream
#: of the -10-like hexamer -- "appropriately positioned" is the whole finding, so
...
class CrypticTranscription:
    ...
    citations: ClassVar[tuple[Citation, ...]] = (
        Citation(
            "AT-tract rule: 103/103 randomized, appropriately positioned AT-tracts "
            "were transcriptionally active",
            "https://academic.oup.com/nar/article/48/9/4891/5820884",
        ),
        Citation(
            "Calibration anchor: the dengue-2 cryptic promoter (-35 TCAACG at nt 53, "
            "17 bp spacer, -10 TTTTTAAT at nt 72) gave ~10^6.6 mRNA copies/ug "
            "uninduced and made the clone uncloneable",
    ...
    def gate(self, slot: ContextSlot) -> bool:
        ...
        and `brief.md:209` marks the cryptic bacterial promoter "On
        (propagation)" in every column.
        """
        return True
    ...
    def _geometry_breaches(self, c: Construct, strand: Strand, role: str) -> list[Breach]:
        ...
        def emit(start: int, length: int, kind: str, message: str) -> None:
            ...
            lo = (start if strand == 1 else n - start - length) % n
            ...
            for spacer in range(self.spacer_min, self.spacer_max + 1):
                ...
                if j in m10:
                    emit(
                        ...
                        f"strand: a -35-like hexamer at {i % n}, a {spacer} bp spacer, then a "
                        f"-10-like hexamer at {j % n}",
                    )
        ...
        for j in sorted(m10):
            for tract in AT_TRACTS:
                ...
                emit(
                    ...
                    "at_tract",
                    f"AT-tract {tract!r} at {start % n} with its 3' end exactly "
                    f"{AT_TRACT_GAP} bp upstream of a -10-like hexamer at {j % n}; "
                    f"103/103 randomized tracts in this position were active",
                )
