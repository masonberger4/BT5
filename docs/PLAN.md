...
> **BT5 writes one canonical GenBank dialect.** Byte-identity with an arbitrary input file is
> impossible in principle, not merely awkward: two files can describe the same molecule with the
> same annotations and differ only in where NCBI, SnapGene and Benchling each wrap a qualifier, and
> one writer emits one layout. Reproducing the input's layout would mean keeping its text and
> editing lines in place — a GenBank *editor* rather than a writer. It would also answer a question
> that does not arise, since what BT5 exports is a **different molecule** from what it imported.
> So the guarantees are: **every annotation survives** (record-level fields included — dropping
> ORGANISM or the date is a defect, not a formatting choice), and **the writer is idempotent**, so a
> diff between two BT5 exports shows only real changes. "Did BT5 touch my backbone?" is answered by
> invariant I9, which compares every backbone base and raises, not by diffing whitespace.

**6a. The exported map annotates itself in three classes.** `PROVIDED BY YOU` — every base and
feature carried through from the input vector, proved byte-identical. `GENERATED BY BT5` — the
designed CDS and the feature describing it. `NOTED BY BT5` — observations about sequence BT5 did
*not* write: backbone liabilities it cannot fix (identical LTRs, an upstream AUG in the user's
5'UTR), assumptions it had to make (a transcription start inferred from a promoter), objectives it
could not evaluate (no annotated 5'UTR), and edits it made to the map itself (a rotated origin, a
dropped feature). Every feature carries `/bt5_origin`, located notes appear as `misc_feature`
marks, and the whole report is in the GenBank `COMMENT`. This third class is what has no equivalent
elsewhere: BT5 never edits the backbone, so when the backbone is the problem the only honest output
is a located warning plus a protocol suggestion. None of it carries a predicted expression level.
...
| **G2** | Lentiviral GenBank round-trips **semantically** incl. origin-spanning `join()` — sequence, topology and every feature location/key/qualifier preserved — with an **idempotent** writer; forbidden sites planted across the CDS/backbone junction **and** across the origin both caught; I9 catches a deliberate backbone edit | all three | the coordinate model is wrong |
